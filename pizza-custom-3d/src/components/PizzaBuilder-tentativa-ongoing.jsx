...
function createBaseByShape(type, size, shape) {
  if (shape === "square") return createSquareBase(type, size);
  if (shape === "triangle") return createTriangleBase(type, size);
  return createBase(type, size); // your original untouched base
}
  ...
  function createBase(type, size) {
    ...
    // 1. Flat centre disc at y=0
    points.push(new THREE.Vector2(0, 0));
    points.push(new THREE.Vector2(innerRadius, 0));
  
    // 2. Crust — full circular arc (not sine) so it's round on top AND bottom
    //    Trace a half-ellipse: from (innerRadius, 0) curving up and over,
    //    back down to (innerRadius + crustWidth, 0)
    const CRUST_STEPS = 58;
    for (let i = 0; i <= CRUST_STEPS; i++) {
      const t = i / CRUST_STEPS;
      const angle = Math.PI - t * Math.PI; // π → 0  (left half of a circle, top-first)
    ...
    const roughnessMap = loader.load("/textures/dough-texture.jpg");
 ...
 function createBaseByShape(type, size, shape) {
  if (shape === "square") return createSquareBase(type, size);
  if (shape === "triangle") return createTriangleBase(type, size);
  return createBase(type, size);
}

function createSquareBase(type, size) {
  const { height, radius } = getBaseDims(type, size);

  const geom = new THREE.BoxGeometry(radius * 2, height, radius * 2);

  const texture = new THREE.TextureLoader().load("/textures/dough-texture.jpg");
  ...
  // important for toppings
  mesh.userData.topY = height;

  return mesh;
}

function createTriangleBase(type, size) {
  const { height, radius } = getBaseDims(type, size);
...
function createSauce(type, size, sType, shape) {
  ...
  if (shape === "square") {
    geom = new THREE.PlaneGeometry(radius * 2, radius * 2);
  } else if (shape === "triangle") {
    const shapeTri = new THREE.Shape();
    shapeTri.moveTo(0, radius);
    shapeTri.lineTo(-radius, -radius);
    shapeTri.lineTo(radius, -radius);
    shapeTri.closePath();
    geom = new THREE.ShapeGeometry(shapeTri);
  } else {
    geom = new THREE.CircleGeometry(radius * 0.95, 64);
  }
...
let color = 0xc23b22; // tomato default
...
if (pizzaShape === "square") {
  ...
  const v1 = { x: 0, z: radius };
  const v2 = { x: -radius, z: -radius };
  const v3 = { x: radius, z: -radius };

  x = (a * v1.x + b * v2.x + c * v3.x) / sum;
  z = (a * v1.z + b * v2.z + c * v3.z) / sum;
} else {
...
mesh.position.set(x, y, z);
  ...
  function snapToRingsIfNeeded(posVec) {
    if (!snapToRingsRef.current || pizzaShape !== "circle") return posVec;
...
function addIngredientAtWorldPos(ing, worldPos) {
  ...
  if (pizzaShape !== "circle") {
    if (pizzaShape === "square") {
      pos.x = THREE.MathUtils.clamp(pos.x, -radius, radius);
      pos.z = THREE.MathUtils.clamp(pos.z, -radius, radius);
    }

    if (pizzaShape === "triangle") {
      const v1 = new THREE.Vector2(0, radius);
      const v2 = new THREE.Vector2(-radius, -radius);
      const v3 = new THREE.Vector2(radius, -radius);
      ...
      const A1 = area(p, v2, v3);
      const A2 = area(v1, p, v3);
      const A3 = area(v1, v2, p);

      const isInside = Math.abs(A - (A1 + A2 + A3)) < 0.01;
