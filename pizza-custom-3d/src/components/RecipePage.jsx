...
export default function RecipePage({ feed, bookmarks, onToggleBookmark }) {
  ...
  // ✅ Firestore IDs are strings — compare as string, not Number(id)
  ...
  return (
    <motion.div
      ...
      style={{ minHeight: "100vh", background: "#0f0f0f", color: "white" }}
    >
      {/* HEADER */}
      <div style={{
        ...
        <span style={{ fontSize: 14, color: "#aaa" }}>Recipe</span>
        <div style={{ width: 20 }} />
      </div>
      ...
      <div style={{ padding: 16 }}>
        <h3 style={{ marginTop: 16 }}>{recipe.author || "Anonymous"}</h3>

        {/* ACTIONS */}
        <div style={{ display: "flex", gap: 16, marginTop: 10, alignItems: "center" }}>
          <div
            onClick={() => onToggleBookmark(recipe.id)}
            style={{ cursor: "pointer", color: bookmarks.includes(recipe.id) ? "#ffd700" : "#fff" }}
          >
            ⭐ {bookmarks.includes(recipe.id) ? "Saved" : "Save"}
          </div>
          <div>⬇ {downloadCounts[recipe.id] || 0}</div>
        </div>
        ...
        <div style={{ marginTop: 20 }}>
          <div
            ...
            style={{
              display: "flex", justifyContent: "space-between", alignItems: "center",
              cursor: "pointer", padding: "10px 0", borderBottom: "1px solid #333",
            }}
          >
          ...
          {showDetails && (
            <div style={{ marginTop: 10, color: "#bbb", lineHeight: 1.6 }}>
              ...
              {recipe.cheeseType && <p><strong>Cheese:</strong> {recipe.cheeseType}</p>}
            </div>
          )}
        </div>
            ...
            {groupedToppings &&
              Object.entries(groupedToppings).map(([name, info], idx) => (
                <span key={idx} style={{
                  ...
                  borderRadius: 999, fontSize: 13, background: "#1a1a1a",
                }}>
                  {name} ×{info.count}
                </span>
              ))}
