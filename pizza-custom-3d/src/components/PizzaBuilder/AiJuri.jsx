...
export default function AiJuri({
  ...
  // Clear the judge result whenever the pizza recipe changes.
  // ingredientCounts is an object so we must stringify it for deep comparison.
  useEffect(() => {
    setJudgeResult(null);
  }, [pizzaShape, baseType, baseSize, sauceType, cheeseType, JSON.stringify(ingredientCounts)]);

  // ── Bake state ────────────────────────────────────────────────────────────
  const [isBaking, setIsBaking] = useState(false);
  const [bakeProgress, setBakeProgress] = useState(0);
  const bakeIntervalRef = useRef(null);
  // Stores { uuid -> THREE.Color } of each topping mesh's original color
  ...
  "suggestion": "one concrete improvement suggestion in one sentence"
    ...
    try {
      ...
      setJudgeResult({
        ...
        score: 50,
        verdict: "The judge couldn't reach a verdict. Possibly too controversial.",
        suggestion: "Try again — the AI chef may have fainted.",
      });
    } finally {
