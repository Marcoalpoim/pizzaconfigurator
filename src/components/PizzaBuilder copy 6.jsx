...
export default function PizzaBuilder({
  ...
  // ── CIRCLE base (your working lathe code, untouched) ──────────────────────
function createCircleBase(type, size) {
  ...
  // --- 1. Flat disc ---
  const discGeom = new THREE.CylinderGeometry(innerRadius, innerRadius, height, 128, 1, false);
  discGeom.computeBoundingBox();
  // Translate so top face sits at y=0
  ...
  // --- 2. Circular crust tube ---
  const crustCentreR = innerRadius + crustWidth * 0.5;
  const STEPS = 128;
  const pts = [];
  for (let i = 0; i < STEPS; i++) {
    const a = (i / STEPS) * Math.PI * 2;
    pts.push(new THREE.Vector3(Math.cos(a) * crustCentreR, 0, Math.sin(a) * crustCentreR));
  }
  const curve3D = new THREE.CatmullRomCurve3(pts, true);

  const tubeRadius = crustWidth * 0.05 + height * 0.8;
  const tubeGeom = new THREE.TubeGeometry(curve3D, 200, tubeRadius, 20, true);

  const discBottom = discGeom.boundingBox.min.y; // = -height
  ...
  group.position.y = -discBottom; // = height, lifts disc bottom to y=0
  group.userData.surfaceY = group.position.y;

  return group;
}

  // ── SHAPED base: crust ring + flat disc, disc top face at y=0 ───────────
function createShapedBase(type, size, shape) {
  const { height, radius } = getBaseDims(type, size);
  const innerRadius = radius * 0.75;
  const crustWidth  = radius * 0.20;

  const mat = makeDoughMat();

  // --- 1. Flat inner disc ---
  const innerShape = makeShape2D(shape, innerRadius * 1.1);
  const discGeom = new THREE.ExtrudeGeometry(innerShape, {
    depth: height,
    bevelEnabled: false,
  });
  ...
  // --- 2. Crust tube ---
  const crustCentreR = innerRadius + crustWidth * 0.5;
  const outlineShape = makeShape2D(shape, crustCentreR);

  const sharpShapes = ["square", "triangle", "diamond"];
  const isSharp = sharpShapes.includes(shape);

  let curve3D;
  if (isSharp) {
    const pts2D = outlineShape.getPoints(256);
    const last = pts2D[pts2D.length - 1];
    const first = pts2D[0];
    if (Math.abs(last.x - first.x) < 1e-4 && Math.abs(last.y - first.y) < 1e-4) {
      pts2D.pop();
    }
    ...
    const pts2D = outlineShape.getPoints(128);
  ...
  // tubeRadius now reflects both the crust width AND the dough thickness,
  // matching how the circle crust grows taller with thicker bases.
  const tubeRadius = crustWidth * 0.05 + height * 0.8;

  const tubeSegments = isSharp ? 400 : 200;
  const tubeGeom = new THREE.TubeGeometry(curve3D, tubeSegments, tubeRadius, 20, true);

  // The disc bottom is at discGeom.boundingBox.min.y (negative, = -height).
  // Place the tube centre at that same Y so the tube bottom aligns with the disc bottom.
  // Tube centre is at 0 by default, tube bottom is at -tubeRadius,
  // so shift up by (discBottom + tubeRadius) to land the tube bottom on discBottom.
  const discBottom = discGeom.boundingBox.min.y; // e.g. -0.08 for medium
  tubeGeom.translate(0, discBottom + tubeRadius, 0);
  ...
  // --- 3. Group and lift so disc bottom sits on y=0 ---
  const group = new THREE.Group();
  group.add(discMesh);
  group.add(crustMesh);
  ...
  // Build a THREE.Shape outline for a given shape name and radius.
  // NOTE: ExtrudeGeometry is built in the XY plane then rotated rotateX(-π/2)
  // so that 2D-X → world-X and 2D-Y → world-(-Z).
  // To make the triangle tip point toward the camera (+Z) we flip the 2D-Y axis:
  // tip at (0, -r) in 2D → world (0, 0, +r) after the rotate.
  function makeShape2D(shape, r) {
    ...
    if (shape === "square") {
      ...
      // Tip at bottom in 2D (-Y) → faces camera (+Z) after rotateX(-π/2)
      s.moveTo(0, -r);                          // tip (front)
      s.lineTo( r * 0.866,  r * 0.5);           // back-right
      s.lineTo(-r * 0.866,  r * 0.5);           // back-left
      s.closePath();
      ...
      // Heart drawn in 2D, flipped so pointy end faces camera after rotateX
      // Pointy bottom in 2D → +Z in world
      const sc = r * 0.55;
      s.moveTo(0, -r * 0.9);            // bottom point (tip, faces camera)
      s.bezierCurveTo(
        -sc * 2, -sc * 0.5,
        -sc * 2,  sc * 1.2,
         0,       sc * 0.8
  ...
  // Dispatcher
  function createBaseByShape(type, size, shape) {
    if (!shape || shape === "circle") return createCircleBase(type, size);
    return createShapedBase(type, size, shape);
  }
  ...
  function randomPointInShape(shape, radius) {
    const r = radius * 0.78; // stay safely inside the crust ring
  ...
  function isInsideShape(shape, x, z, r) {
    ...
    if (shape === "triangle") {
      // Matches the flipped makeShape2D triangle:
      // tip at z=+r (front), back edge at z=-r*0.5
      // In world space: 2D-Y → -Z, so 2D tip (0,-r) → world z=+r
      const v1 = { x: 0,           z:  r };         // tip (front)
      const v2 = { x:  r * 0.866,  z: -r * 0.5 };  // back-right
      const v3 = { x: -r * 0.866,  z: -r * 0.5 };  // back-left
      return pointInTriangle(x, z, v1, v2, v3);
    }
    if (shape === "star") {
      // Approximate with inner circle of the star
      return x * x + z * z <= (r * 0.42) * (r * 0.42);
    }
    if (shape === "heart") {
      // Approximate with a circle for placement purposes
      return x * x + z * z <= (r * 0.7) * (r * 0.7);
    }
    return x * x + z * z <= r * r;
  }
  ...
  function getSauceY()   { return getBaseSurfaceY() + SAUCE_OFFSET; }
  function getCheeseY()  { return getBaseSurfaceY() + CHEESE_OFFSET; }
  function getToppingY() { return getBaseSurfaceY() + TOPPING_OFFSET; }

  // Keep old name for drag/drop callers
  function getToppingSurfaceY() { return getToppingY(); }

  function snapToRingsIfNeeded(posVec) {
    if (!snapToRingsRef.current || pizzaShapeRef.current !== "circle") return posVec;
    ...
    if (ing.id === "pepperoni" && modelsRef.current.pepperoni) {
      ...
      switch (ing.kind) {
        ...
        case "pineapple": {
          ...
          const pTex = new THREE.TextureLoader().load("/textures/pineapple.jpg");
          mesh = new THREE.Mesh(geom, new THREE.MeshStandardMaterial({ map: pTex, roughness: 0.55, metalness: 0.02 }));
  ...
  function generateRandomPizza() {
    ...
    setPizzaShape(rand(["circle", "square", "triangle", "diamond", "oval", "star", "heart"]));
  ...
  useEffect(() => {
    ...
    const base = createBaseByShape(baseType, baseSize, pizzaShape);
...
useEffect(() => {
  ...
  for (let i = 0; i < 180; i++) {
    group.add(createCheeseBlob(pizzaShape, cheeseRadius, cheeseY, cheeseType));
  }
  ...
  const buildRecipe = (image) => ({
    ...
    baseType, baseSize, pizzaShape, cheeseType, sauceType,
        ...
        <div className="config-container">
          <aside className={`config-modal ${showConfig ? "open" : "closed"}`}>
            ...
            <PizzaControls
              ...
              pizzaShape={pizzaShape} setPizzaShape={setPizzaShape}
